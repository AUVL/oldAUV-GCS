...
namespace MissionPlanner
{
    public class CurrentState : ICloneable
    {
        ...
        public float ch3percent
        {
            get
            {
                ...
                try
                {
                    if (AUV_GCS.MainForm.comPort.MAV.param.ContainsKey("RC3_MIN") &&
                        AUV_GCS.MainForm.comPort.MAV.param.ContainsKey("RC3_MAX"))
                    {
                        return
                            (int)
                                (((ch3out - AUV_GCS.MainForm.comPort.MAV.param["RC3_MIN"].Value)/
                                  (AUV_GCS.MainForm.comPort.MAV.param["RC3_MAX"].Value - AUV_GCS.MainForm.comPort.MAV.param["RC3_MIN"].Value))*
                                 100);
                    }
                    else
                    {
	...
	public float crit_AOA
        {
            get
            {
                try
                {
                    if (AUV_GCS.MainForm.comPort.MAV.param.ContainsKey("AOA_CRIT"))
                    {
                        return
                            (int)
                                (AUV_GCS.MainForm.comPort.MAV.param["AOA_CRIT"].Value);
                    }
                    else
                    {
                        return 25;
	...
	float _crit_aoa = 25;
        ...
        public float GeoFenceDist
        {
            get
            {
                try
                {
                    ...
                    var R = 6371e3;
                    // close loop
                    var list = AUV_GCS.MainForm.comPort.MAV.fencepoints.ToList();
                    if (list.Count > 0)
                    {
        ...
        // current firmware
        public AUV_GCS.MainForm.Firmwares firmware = AUV_GCS.MainForm.Firmwares.ArduCopter2;
        ...
        {
            get { return (AUV_GCS.MainForm.comPort.BaseStream.IsOpen || AUV_GCS.MainForm.comPort.logreadmode); }
        }
        ...
        {
            UpdateCurrentSettings(bs, false, AUV_GCS.MainForm.comPort, AUV_GCS.MainForm.comPort.MAV);
        }
                ...
                if (DateTime.Now > lastupdate.AddMilliseconds(50) || updatenow) // 20 hz
                {
                    ...
                    if (mavLinkMessage != null)
                    {
                        ...
                        else
                        {
                            ...
                            if (oldmode != mode && AUV_GCS.MainForm.speechEnable && AUV_GCS.MainForm.comPort.MAV.cs == this &&
                                Settings.Instance.GetBoolean("speechmodeenabled"))
                            {
                                AUV_GCS.MainForm.speechEngine.SpeakAsync(Common.speechConversion(""+ Settings.Instance["speechmode"]));
                            }
                        }
                    }
                    ...
                    if (mavLinkMessage != null)
                    {
                        ...
                        if (oldwp != wpno && AUV_GCS.MainForm.speechEnable && AUV_GCS.MainForm.comPort.MAV.cs == this &&
                            Settings.Instance.GetBoolean("speechwaypointenabled"))
                        {
                            AUV_GCS.MainForm.speechEngine.SpeakAsync(Common.speechConversion(""+ Settings.Instance["speechwaypoint"]));
                        }
