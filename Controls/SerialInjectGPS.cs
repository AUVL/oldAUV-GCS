...
namespace MissionPlanner
{
    public partial class SerialInjectGPS : UserControl, IActivate, IDeactivate
    {
        ...
        {
            if (AUV_GCS.MainForm.instance.IsDisposed)
                threadrun = false;

            AUV_GCS.MainForm.instance.BeginInvoke((MethodInvoker) delegate
            {
                ...
                try
                {
                    switch (CMB_serialport.Text)
                    {
                        case "NTRIP":
                            ...
                            ((CommsNTRIP) comPort).lat = AUV_GCS.MainForm.comPort.MAV.cs.HomeLocation.Lat;
                            ((CommsNTRIP) comPort).lng = AUV_GCS.MainForm.comPort.MAV.cs.HomeLocation.Lng;
                            ((CommsNTRIP) comPort).alt = AUV_GCS.MainForm.comPort.MAV.cs.HomeLocation.Alt;
        ...
        private static void ExtractBasePos(int seen)
        {
            try
            {
                if (seen == 1005)
                {
                    ...
                    AUV_GCS.MainForm.comPort.MAV.cs.MovingBase = new Utilities.PointLatLngAlt(baseposllh[0]*Utilities.rtcm3.R2D,
                        baseposllh[1]*Utilities.rtcm3.R2D, baseposllh[2]);
                ...
                else if (seen == 1006)
                {
                    ...
                    AUV_GCS.MainForm.comPort.MAV.cs.MovingBase = new Utilities.PointLatLngAlt(baseposllh[0], baseposllh[1],
                        baseposllh[2]);
        ...
        {
            foreach (var port in AUV_GCS.MainForm.Comports)
            {
        ...
        {
            if (AUV_GCS.MainForm.comPort.MAV.cs.MovingBase == null)
            {
            ...
            if (InputBox.Show("Enter Location", "Enter a friendly name for this location.", ref location) ==
                DialogResult.OK)
            {
                var basepos = AUV_GCS.MainForm.comPort.MAV.cs.MovingBase;
