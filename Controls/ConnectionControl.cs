...
namespace MissionPlanner.Controls
{
    public partial class ConnectionControl : UserControl
    {
        ...
        public void UpdateSysIDS()
        {
            ...
            foreach (var port in AUV_GCS.MainForm.Comports.ToArray())
            {
        ...
        private void CMB_sysid_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            foreach (var port in AUV_GCS.MainForm.Comports)
            {
                if (port == temp.port)
                {
                    AUV_GCS.MainForm.comPort = port;
                    AUV_GCS.MainForm.comPort.sysidcurrent = temp.sysid;
                    AUV_GCS.MainForm.comPort.compidcurrent = temp.compid;

                    if (AUV_GCS.MainForm.comPort.MAV.param.Count == 0 && !(Control.ModifierKeys == Keys.Control))
                        AUV_GCS.MainForm.comPort.getParamList();
                }
            }
        }
