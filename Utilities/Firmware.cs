...
namespace MissionPlanner.Utilities
{
    public class Firmware
    {
        ...
        {
            if (AUV_GCS.MainForm.comPort.BaseStream is SerialPort)
            {
                try
                {
                    updateProgress(-1, "Look for HeartBeat");
                    // check if we are seeing heartbeats
                    AUV_GCS.MainForm.comPort.BaseStream.Open();
                    AUV_GCS.MainForm.comPort.giveComport = true;

                    if (AUV_GCS.MainForm.comPort.getHeartBeat().Length > 0)
                    {
                        updateProgress(-1, "Reboot to Bootloader");
                        AUV_GCS.MainForm.comPort.doReboot(true, false);
                        AUV_GCS.MainForm.comPort.Close();
                    }
                    else
                    {
                        updateProgress(-1, "No HeartBeat found");
                        AUV_GCS.MainForm.comPort.BaseStream.Close();
...
//VR boards have no tone alarm
        ...
        { 
            ...
            AUV_GCS.MainForm.comPort.giveComport = false;
            return true;
        }
