...
namespace MissionPlanner.Utilities
{
    public class Proximity : IDisposable
    {
        ...
        private void Temp_Paint(object sender, PaintEventArgs e)
        {
            ...
            {
                case AUV_GCS.MainForm.Firmwares.ArduCopter2:
                    var imw = size/2;
                    e.Graphics.DrawImage(global::AUV_GCS.Properties.Resources.quadicon, midx - imw, midy - imw, size, size);
                    break;
            }
        ...
        private bool messageReceived(MAVLinkMessage arg)
        {
            ...
            if (arg.msgid == (uint)MAVLINK_MSG_ID.DISTANCE_SENSOR)
            {
                ...
                else
                {
                    if (!temp.IsDisposed)
                    {
                        AUV_GCS.MainForm.instance.Invoke((MethodInvoker)delegate { temp.Show(); });
                    }
                }
            }
