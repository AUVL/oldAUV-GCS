...
namespace MissionPlanner.Comms
{
    public class CommsSerialScan
    {
        ...
        {
            if (AUV_GCS.MainForm.instance == null)
            {
                AUV_GCS.MainForm.comPort.BaseStream = port;
                AUV_GCS.MainForm.comPort.Open(false);
            }
            else
            {
                if (AUV_GCS.MainForm.instance.InvokeRequired)
                {
                    AUV_GCS.MainForm.instance.BeginInvoke(
                        (MethodInvoker) delegate()
                        {
                            MAVLinkInterface mav = new MAVLinkInterface();
                            mav.BaseStream = port;
                            AUV_GCS.MainForm.instance.doConnect(mav, "preset", "0");
                            AUV_GCS.MainForm.Comports.Add(mav);
                        });
                }
                else
                {
