...
namespace MissionPlanner
{
    ...
    public class Common
    {
        ...
        public static List<KeyValuePair<int, string>> getModesList(CurrentState cs)
        {
            ...
            if (cs.firmware == AUV_GCS.MainForm.Firmwares.PX4)
            {
                /*
union px4_custom_mode {
    ...
    float data_float;
            ...
            else if (cs.firmware == AUV_GCS.MainForm.Firmwares.ArduPlane)
            {
            ...
            else if (cs.firmware == AUV_GCS.MainForm.Firmwares.Ateryx)
            {
                var flightModes = Utilities.ParameterMetaDataRepository.GetParameterOptionsInt("FLTMODE1",
                    cs.firmware.ToString()); //same as apm
                return flightModes;
            }
            else if (cs.firmware == AUV_GCS.MainForm.Firmwares.ArduCopter2)
            {
                var flightModes = Utilities.ParameterMetaDataRepository.GetParameterOptionsInt("FLTMODE1",
                    cs.firmware.ToString());
                return flightModes;
            }
            else if (cs.firmware == AUV_GCS.MainForm.Firmwares.ArduRover)
            {
                var flightModes = Utilities.ParameterMetaDataRepository.GetParameterOptionsInt("MODE1",
                    cs.firmware.ToString());
                return flightModes;
            }
            else if (cs.firmware == AUV_GCS.MainForm.Firmwares.ArduTracker)
            {
                var temp = new List<KeyValuePair<int, string>>();
        ...
        public static Form LoadingBox(string title, string promptText)
        {
            ...
            System.ComponentModel.ComponentResourceManager resources =
                new System.ComponentModel.ComponentResourceManager(typeof(AUV_GCS.MainForm));
        ...
        {
            if (AUV_GCS.MainForm.comPort.MAV.cs.wpno == 0)
            {
                input = input.Replace("{wpn}", "Home");
            }
            else
            {
                input = input.Replace("{wpn}", AUV_GCS.MainForm.comPort.MAV.cs.wpno.ToString());
            }

            input = input.Replace("{asp}", AUV_GCS.MainForm.comPort.MAV.cs.airspeed.ToString("0"));

            input = input.Replace("{alt}", AUV_GCS.MainForm.comPort.MAV.cs.alt.ToString("0"));

            input = input.Replace("{wpa}", AUV_GCS.MainForm.comPort.MAV.cs.targetalt.ToString("0"));

            input = input.Replace("{gsp}", AUV_GCS.MainForm.comPort.MAV.cs.groundspeed.ToString("0"));

            input = input.Replace("{mode}", AUV_GCS.MainForm.comPort.MAV.cs.mode.ToString());

            input = input.Replace("{batv}", AUV_GCS.MainForm.comPort.MAV.cs.battery_voltage.ToString("0.00"));

            input = input.Replace("{batp}", (AUV_GCS.MainForm.comPort.MAV.cs.battery_remaining).ToString("0"));

            input = input.Replace("{vsp}", (AUV_GCS.MainForm.comPort.MAV.cs.verticalspeed).ToString("0.0"));

            input = input.Replace("{curr}", (AUV_GCS.MainForm.comPort.MAV.cs.current).ToString("0.0"));

            input = input.Replace("{hdop}", (AUV_GCS.MainForm.comPort.MAV.cs.gpshdop).ToString("0.00"));

            input = input.Replace("{satcount}", (AUV_GCS.MainForm.comPort.MAV.cs.satcount).ToString("0"));

            input = input.Replace("{rssi}", (AUV_GCS.MainForm.comPort.MAV.cs.rssi).ToString("0"));

            input = input.Replace("{disthome}", (AUV_GCS.MainForm.comPort.MAV.cs.DistToHome).ToString("0"));

            input = input.Replace("{timeinair}",
                (new TimeSpan(0, 0, 0, (int)AUV_GCS.MainForm.comPort.MAV.cs.timeInAir)).ToString());

            try
            {
                object thisBoxed = AUV_GCS.MainForm.comPort.MAV.cs;
        ...
        public static GMapMarker getMAVMarker(MAVState MAV)
        {
            ...
            else if (MAV.cs.firmware == AUV_GCS.MainForm.Firmwares.ArduTracker)
            {
                return (new GMapMarkerAntennaTracker(portlocation, MAV.cs.yaw,
                    MAV.cs.target_bearing));
            }
            else if (MAV.cs.firmware == AUV_GCS.MainForm.Firmwares.ArduCopter2 || MAV.aptype == MAVLink.MAV_TYPE.QUADROTOR)
            {
    ...
    {
        private static readonly Bitmap icong = new Bitmap(global::AUV_GCS.Properties.Resources.FW_icons_2013_logos_01, new Size(40, 40));
        private static readonly Bitmap iconr = new Bitmap(global::AUV_GCS.Properties.Resources.FW_icons_2013_logos_011, new Size(40, 40));
        private static readonly Bitmap icono = new Bitmap(global::AUV_GCS.Properties.Resources.FW_icons_2013_logos_012, new Size(40, 40));
    ...
    public class GMapMarkerRover : GMapMarker
    {
        static readonly System.Drawing.Size SizeSt =
            new System.Drawing.Size(global::AUV_GCS.Properties.Resources.rover.Width,
                global::AUV_GCS.Properties.Resources.rover.Height);
        ...
        public override void OnRender(Graphics g)
        {
            ...
            g.DrawImageUnscaled(global::AUV_GCS.Properties.Resources.rover,
                global::AUV_GCS.Properties.Resources.rover.Width/-2,
                global::AUV_GCS.Properties.Resources.rover.Height/-2);
    ...
    {
        static readonly System.Drawing.Size SizeSt =
            new System.Drawing.Size(global::AUV_GCS.Properties.Resources.boat.Width,
                global::AUV_GCS.Properties.Resources.boat.Height);
        ...
        public override void OnRender(Graphics g)
        {
            ...
            g.DrawImageUnscaled(global::AUV_GCS.Properties.Resources.boat,
                global::AUV_GCS.Properties.Resources.boat.Width / -2,
                global::AUV_GCS.Properties.Resources.boat.Height / -2);
    ...
    public class GMapMarkerSub : GMapMarker
    {
        ...
        private static Bitmap imagecache = new Bitmap(global::AUV_GCS.Properties.Resources.sub, SizeSt);
    ...
    {
        private readonly Bitmap icon = global::AUV_GCS.Properties.Resources.planeicon;
    ...
    {
        private readonly Bitmap icon = global::AUV_GCS.Properties.Resources.quadicon;
    ...
    {
        private readonly Bitmap icon = global::AUV_GCS.Properties.Resources.redsinglecopter2;
    ...
    {
        private readonly Bitmap icon = global::AUV_GCS.Properties.Resources.heli;
    ...
    {
        private readonly Bitmap icon = global::AUV_GCS.Properties.Resources.Antenna_Tracker_01;
